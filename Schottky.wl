...
MobiusMatrixToTransformation[{{a_, b_}, {c_, d_}}] :=
  Module[{f},
    f[z_] := (a z + b) / (c z + d);
    Return[f];
  ]
...
MobiusMatrixToCircle[{{a0_, b0_}, {c0_, d0_}}] :=
  If[a0 == 0,
    Return[{-d0 / c0, 0, b0 / c0}]
    ,
    With[{b = b0 / a0, c = c0 / a0, d = d0 / a0},
      Return[{-d / c, 1 / c, b / c - d / c^2}]
    ]
  ];

MobiusCircleToMatrix[{p0_, p1_, \[Phi]_}] :=
  {{p1, -p0 p1 + \[Phi]}, {1, -p0}};
...
MobiusTransformationToMatrix[f_] :=
  Module[{InfPos = 0, list = {1, 0, I}},
    ...
    If[InfPos == 0,
      With[{z1 = 1, z2 = 0, z3 = I},
        With[{w1 = f[z1], w2 = f[z2], w3 = f[z3]},
          Return[MobiusTransformationToMatrixEval[z1, z2, z3, w1, w2,
             w3]];
        ]
      ]
      ,
      With[{z1 = list[[InfPos]], z2 = Delete[list, InfPos][[1]], z3 =
         Delete[list, InfPos][[2]]},
        With[{w2 = f[z2], w3 = f[z3]},
          Return[MobiusTransformationToMatrixEvalInf[z1, z2, z3, w2, 
            w3]]
        ]
      ]
    ]
...
GenerationGetSimpleGeneratorMap[c1_, c2_, \[Phi]_:0] :=
  MobiusMatrixToTransformation[GenerationGetSimpleGeneratorMatrix[c1,
     c2, \[Phi]]];

GenerationGetSimpleGroupElement[c1_, c2_, \[Phi]_:0] :=
  Return[{c1, c2, GenerationGetSimpleGeneratorMatrix[c1, c2, \[Phi]]}
    ];
...
GenerationGetSimpleSchottkyGroup[clist_] :=
  If[Length[#] == 3,
      ...
      ,
      GenerationGetSimpleGroupElement[#[[1]], #[[2]]]
    ]& /@ clist;

(*Getters and Setters*)

IOGetCirclePair[{c1_, c2_, A_}] :=
  {c1, c2};

IOGetMatrix[{c1_, c2_, A_}] :=
  A;
...
GraphicsDisplayCirclePair[circpair_, color_] :=
  Return[{Style[ComplexCircle[IOGetCirclePair[circpair][[1]]], color],
     Style[ComplexCircle[IOGetCirclePair[circpair][[2]]], {color, Dashed}
    ]}]
...
GraphicsDisplayCover[circpairs_, n_] :=
  Module[{ret = {}, i},
    With[{maps = GroupGetAllMaps[circpairs, n]},
      For[i = 1, i <= Length[circpairs], i++,
        AppendTo[ret, Style[GraphicsDisplayCirclePair[circpairs[[i]],
           Hue[(i - 1) / Length[circpairs]]], {Thick}]& /@ maps];
        AppendTo[ret, Style[ComplexCircle[TransformCircle[IOGetCirclePair[
          circpairs[[i]]][[1]], #]], Hue[(i - 1) / Length[circpairs]]]& /@ maps
          ];
      ];
      Return[ret];
    ]
...
BranchCutGetFixedPointInversionMatrix[{c1_, c2_, M_}, \[Lambda]_:1] :=
  With[{p = BranchCutGetFixedPoints[{c1, c2, M}]},
    If[Length[p] == 1,
...
BranchCutGetNormalizedFixedPointInversionMatrix[C_, p_, \[Lambda]_:1] :=
...
Options[BranchCutGetConcentricBCycleCurve] = {"\[Phi]" -> 0, "range" 
  -> {0, 1}};

BranchCutGetConcentricBCycleCurve[{c1_, c2_, M_}, OptionsPattern[]] :=
  Module[{p, q, \[Gamma]},
    q = M[[1, 1]] / M[[2, 2]];
...
Options[BranchCutGetBCycleCurve] = {"\[Lambda]" -> 1, "\[Phi]" -> 0, 
  "range" -> {0, 1}};
